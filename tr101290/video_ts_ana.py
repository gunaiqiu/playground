...
def find_len_of_packet(folder,file_list,i):
    ...
    # 定义一个常量len1，值为188
    # len2 = 204
    # 注释掉的代码行，定义了一个未使用的常量len2，值为204
    with open(os.path.join(folder,file_list[i]), 'rb') as f:
        # 使用'rb'模式打开文件，即二进制读取模式
        # 拼接文件夹路径和文件名列表中的第i个文件名，得到完整的文件路径
        section = f.read(len1)
        # 读取文件的前len1个字节，赋值给变量section
        while len(section) == len1 :
            # 当读取到的section长度等于len1时，执行循环体
            # for c in section:
            # 注释掉的代码行，用于遍历section中的每个字符，但在当前代码块中未使用
            if section[0] == 0x47:
                # 如果section的第一个字节是0x47，则执行以下代码
                print("%03d "%(start))
                # 打印变量start的值，格式化输出为三位数，不足三位前面补0
            # offset = offset+1
            # 注释掉的代码行，用于更新offset的值，但在当前代码块中未定义offset变量
            section = f.read(188)
            # 继续读取文件的下一个len1个字节，更新section的值
            start += 1
    # start自增1
# 找ts流有什么类型的包
"""
